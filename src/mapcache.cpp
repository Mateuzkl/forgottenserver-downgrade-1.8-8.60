...
void MapCache::flush()
{
	...
	double itemHitRate = totalItemRequests > 0 ? (static_cast<double>(itemCacheHits) / totalItemRequests * 100.0) : 0.0;
	double tileHitRate = totalTileRequests > 0 ? (static_cast<double>(tileCacheHits) / totalTileRequests * 100.0) : 0.0;

	itemCache.clear();
	tileCache.clear();

	LOG_MAPCACHE(fmt::format(
	    "Map cache flushed: [\033[1;36m{}\033[0m] unique items, [\033[1;36m{}\033[0m] unique tiles deduplicated",
	    itemCount, tileCount));
	LOG_MAPCACHE(fmt::format(
	    "Cache hit rates: Items [\033[1;33m{:.2f}%\033[0m] (\033[1;36m{}\033[0m/\033[1;36m{}\033[0m), Tiles [\033[1;33m{:.2f}%\033[0m] (\033[1;36m{}\033[0m/\033[1;36m{}\033[0m)",
	    itemHitRate, itemCacheHits.load(), totalItemRequests, tileHitRate, tileCacheHits.load(), totalTileRequests));
...
std::shared_ptr<BasicTile> MapCache::parseBasicTile(void* loaderptr, const void* nodeptr, uint8_t& xOffset,
                                                    uint8_t& yOffset)
{
	OTB::Loader& loader = *static_cast<OTB::Loader*>(loaderptr);
...
std::unique_ptr<Tile> createTileFromBasic(const std::shared_ptr<BasicTile>& basicTile, uint16_t x, uint16_t y,
                                          uint8_t z, Houses& houses)
{
	if (!basicTile) {
		return nullptr;
	}
