...
void ProtocolSpectator::kick(const DataList& _kicks)
{
	for (auto& it : _kicks)
		for (auto& spectator : spectators)
			if (it.second == spectator->getIP()) spectator->disconnect();
}
...
void ProtocolSpectator::spectatorSay(ProtocolGame_ptr spectator, std::string_view text)
{
	if (text[0] == '/') {
		...
		if (cmd == "list" || cmd == "show" || cmd == "spectators") {
			...
			for (const auto& it : spectators) s << " " << it->getSpectatorName() << ",";
			s.seekp(-1, s.cur);
			s << ".";
			sendCastMessage("", s.str().substr(0, 255), TALKTYPE_CHANNEL_O, spectator);
		} else if (cmd == "name" || cmd == "nick") {
			if (sv.size() == 1) return sendCastMessage("", "Usage: /nick new name.", TALKTYPE_CHANNEL_O, spectator);
			...
			sendCastMessage("", spectator->getSpectatorName() + " has changed name to " + nickname + ".",
			                TALKTYPE_CHANNEL_O);
	...
	bool muted = false;
	for (auto& it : mutes)
		if (it.second == spectator->getIP()) muted = true;
...
void ProtocolSpectator::sendCastMessage(const std::string& author, std::string_view text, SpeakClasses type,
                                        ProtocolGame_ptr target /* = nullptr */)
{
	if (!target) return sendChannelMessage(author, text, type, CHANNEL_CAST);
