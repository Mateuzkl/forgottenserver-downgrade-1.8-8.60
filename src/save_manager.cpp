...
void SaveManager::saveAll()
{
	if (saving.exchange(true)) {
		LOG_INFO(fmt::format(">> {}: {}", fmt::format(fg(fmt::color::magenta), "SaveManager"),
	...
	if (failCount > 0) {
		LOG_INFO(fmt::format(
		    ">> {}: Saved {} players ({} failed) in {}", fmt::format(fg(fmt::color::magenta), "SaveManager"),
		    fmt::format(fg(fmt::color::lime_green), "{}", playerCount),
		    fmt::format(fg(fmt::color::red), "{}", failCount), fmt::format(fg(fmt::color::cyan), "{}ms", durationMs)));
	} else {
...
bool SaveManager::savePlayer(Player* player)
{
	...
	if (success) {
		LOG_INFO(fmt::format(">> {}: Player {} saved in {}", fmt::format(fg(fmt::color::magenta), "SaveManager"),
...
void SaveManager::saveMapAsync()
{
	...
	g_threadPool.detach_task([]() {
		...
		if (mapSaved) {
			LOG_INFO(fmt::format(">> {}: Map saved in {}", fmt::format(fg(fmt::color::magenta), "SaveManager"),
