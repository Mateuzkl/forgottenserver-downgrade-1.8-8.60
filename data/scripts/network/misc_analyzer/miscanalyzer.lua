...
-- Misc Analyzer - sends charm/imbuement/special skill activations to OTCv8 clients
...
local function canReceiveOpcode(player, opcode)
	...
	if otcOpcodes[opcode] then
		return isExtendedOtcv8(player) or isOtc(player)
	end
	return isExtendedOtcv8(player)
end
